...
else if(responseContext.db_flag =='x2' ) //completed
...
////function to fetch the list of activites based on completed/delay/progress
  function fetchactivities(status,flag){
	  console.log('in fetch activity fucntion') ;
	   ...
	   var FetchQuery ="SELECT CONCAT(CONCAT(USECASE,':') ,PHASE_DETAIL) USECASE FROM  MHPSJP3 where A_E_D is not null ";
       ...
       var out = "";
		var i;
		 ...
		 out = '<table>'
		  ...
		  {
			out += '<tr><td style="width:50%">'+ data[i].USECASE + '<br/td></tr>';
		 ...
		 console.log(x);
		  ...
		  console.log(out);
		  	...
		  	$("#timeStamp").append("<div class='direct-chat-msg doted-border'><div class='direct-chat-info clearfix'><span class='direct-chat-name pull-left'><img style='margin-left: -58px;width: 54px;border-radius: 55%;' src='Capture.PNG' alt='User Avatar' class='img-circle' /></span></div><div style='margin-top: -40px' class='direct-chat-text'>" +'Below is the list of activities'+ "</div><div id='timeStamp' class='direct-chat-info clearfix'><span id='time' class='direct-chat-timestamp pull-right'>" +formatted5+ "</span></div></div>");

      	  $("#timeStamp").append("<div class='direct-chat-msg doted-border'><div class='direct-chat-info clearfix'><span class='direct-chat-name pull-left'><img style='margin-left: -58px;width: 54px;border-radius: 55%;' src='Capture.PNG' alt='User Avatar' class='img-circle' /></span></div> <b>"+ out +".</b>.</div><div id='timeStamp' class='direct-chat-info clearfix'><span id='time' class='direct-chat-timestamp pull-right'>" +formatted5+ "</span></div></div>");
		 //change123
 ...
 ////function to fetch the list of activites based on completed/delay/progress
  function fetchphasedetail(u_flag,p_flag){
	  ...
	  var p_flag = '基本と詳細設計';
	    var FetchQuery ="SELECT PHASE_DETAIL FROM  MHPSJP3 where USECASE = '" + u_flag +  "'and PHASE= '" + p_flag +"'" ;
        ...
        contentType: "application/json",
        dataType:'json',
        url: 'http://mhpspm1.mybluemix.net/fetchphasedetail',                      
